...
# Specify output file
outfile <- "models/pibkc/test/bkc.dat"
# Create an empty file (overwrite if exists)
file.create(outfile)

# Write header and values
cat("# Pribilof islands blue king crab pop dy model\n", file = outfile)
...
in_stm<-read.csv("models/pibkc/test/size_trans_pibkc.csv",header=F)
...
# Specify output file
outfile <-  "models/pibkc/test/bkc.pin"
file.create(outfile)

# Write header and values
cat("# Pribilof Islands blue king crab pop dy model .pin file\n", file = outfile)
cat(paste("# Generated on: ", Sys.Date()),"\n", file = outfile, append = TRUE)
...
cat(rep(0,17), file = outfile, append = TRUE,"\n")
