...
# Specify output file
outfile <- "models/pirkc/test/rkc.dat"
# Create an empty file (overwrite if exists)
file.create(outfile)

# Write header and values
cat("# Pribilof islands red king crab pop dy model\n", file = outfile)
...
input_p_molt<-1 - (1/(1+exp(-0.117*(in_sizes-165))))
cat(input_p_molt, file = outfile, append = TRUE,"\n")

in_stm<-read.csv("models/pirkc/size_trans_pirkc.csv",header=F)
...
# Specify output file
outfile <-  "models/pirkc/test/rkc.pin"
file.create(outfile)

# Write header and values
cat("# Pribilof Islands red king crab pop dy model .pin file\n", file = outfile)
cat(paste("# Generated on: ", Sys.Date()),"\n", file = outfile, append = TRUE)
...
cat(rep(0,6), file = outfile, append = TRUE,"\n")
