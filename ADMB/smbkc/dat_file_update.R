...
n_at_size_dat_in<-n_at_size_dat[,-c(1,ncol(n_at_size_dat))]
...
use_n<-matrix(0,ncol=ncol(n_at_size_dat_in),nrow=length(allyr))
rownames(use_n)<-allyr
use_n_sc<-use_n
in_sizes<-as.numeric(colnames(n_at_size_dat_in))

for(x in 1:length(allyr))
{
  use_n[x,]  <- unlist(n_at_size_dat_in[(match(allyr[x],as.numeric(rownames(n_at_size_dat_in)))),])
...
# Specify output file
outfile <- "models/smbkc/test/bkc.dat"
# Create an empty file (overwrite if exists)
file.create(outfile)

# Write header and values
cat("# Saint Matthews blue king crab pop dy model\n", file = outfile)
...
cat(in_sizes, file = outfile, append = TRUE,"\n")
...
input_p_molt<-1 - (1/(1+exp(-0.142*(in_sizes-137.8))))
cat(input_p_molt, file = outfile, append = TRUE,"\n")

in_stm<-read.csv("models/smbkc/test/size_trans_smbkc.csv",header=F)
...
cat(40, file = outfile, append = TRUE,"\n")
...
cat(c(0.001), file = outfile, append = TRUE,"\n")
...
cat(0.0, file = outfile, append = TRUE,"\n")
...
# Specify output file
outfile <-  "models/smbkc/test/bkc.pin"
file.create(outfile)

# Write header and values
cat("# Saint Matthews island blue king crab pop dy model .pin file\n", file = outfile)
cat(paste("# Generated on: ", Sys.Date()),"\n", file = outfile, append = TRUE)

cat("# log_n_init", file = outfile, append = TRUE,"\n")
cat(log(rep(mean(use_n[1,]),length(in_sizes))), file = outfile, append = TRUE,"\n")
...
cat(10, file = outfile, append = TRUE,"\n")
...
cat(c(-1.49), file = outfile, append = TRUE,"\n")
cat("# prop_rec", file = outfile, append = TRUE,"\n")
cat(c(12.4,12.9), file = outfile, append = TRUE,"\n")
...
cat(rep(0,26), file = outfile, append = TRUE,"\n")
